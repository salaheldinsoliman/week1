...
template sudoku() {
    ...
    //RangeProof inRange[81]; //= RangeProof ();
    //RangeProof inRange2[81];// = RangeProof ();
    //component array = all();

     component inRange[162];
    for (var i = 0 ; i< 162 ; i++ )
    {
            
           inRange[i] = RangeProof();
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            //assert(puzzle[i][j]<=9); //[assignment] change assert() to use your created RangeProof instead
             ...
             inRange[k].min <== 0;
              inRange[k].max <== 9;
              inRange[k].in <== puzzle[i][j];
              assert (inRange[k].out == 0);

             k = k+1 ;

             inRange[k].min <== 0;
             inRange[k].max <== 9;
             inRange[k].in <== solution[i][j];
             assert (inRange[k].out == 0);
